var AIS = (function() {
	...
	var colorIterator = function(){
	    // Aufgabe 2
	    thiss.iterate = this.iterate + 1 % AISShipColors.length;
	    this.color = AISShipColors[this.iterate];

	    return colorIterator();
	};
	colorIterator.prototype.iterate = 0;
	colorIterator.prototype.getColor = AISShipColors[0];
	
	// Ship Konstruktor 
	var Ship = function(mmsi, latitude, longitude, courseOverGround) {
	    // Aufgabe 3
	    Ship.prototype.mmsi = mmsi;
	    Ship.prototype.latitude = latitude;
	    Ship.prototype.longitude = longitude;
	    Ship.prototype.courseOverGround = courseOverGround;
	};
	...
	ShipList.prototype.filter = function(filterFunction) {
	    ...
	    var newList = [];
	    for (var i in ShipList.list) {
	        if(filterFunction(ShipList.list[i]))
	            newList.push(ShipList.list[i]);
	    }
	    ShipList.list = newList;
	};
